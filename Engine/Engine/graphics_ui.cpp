...
GUIRect::GUIRect(float x1, float y1, float x2, float y2, colour_t colours[3], GUIText* pGUITextInput)
...
GUIText::GUIText(const std::string sText, const colour_t colours[3], const float x1, const float y1, const float x2, const float y2, const bool drawBackground, const colour_t cBackground)
...
const bool GUISprite::isClickable(const Vec2f& vf_)
{
	if (vf_.x > vf1.x && vf_.y > vf1.y && vf_.x < vf2.x && vf_.y < vf2.y)
	{
		...
		return ((colour_t)(colour & UINT32_ALPHA_CHANNEL)) > ALPHA_THRESHOLD;
	}
